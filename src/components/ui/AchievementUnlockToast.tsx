...
import { ACHIEVEMENTS, ACHIEVEMENT_CATEGORY_STYLES, getAchievementIcon } from "@/data/achievements";
...
export function AchievementUnlockToast() {
  ...
  const style = achievement ? ACHIEVEMENT_CATEGORY_STYLES[achievement.category] : null;
      ...
      <AnimatePresence mode="wait">
        {achievement && style && (
          <motion.div
            ...
            className={cn(
              "pointer-events-auto rounded-xl overflow-hidden ring-1 shadow-2xl bg-gradient-to-r",
              style.ring,
              style.gradient
            )}
          >
            <div className="flex items-center gap-3 p-3">
              <div
                className={cn(
                  "w-12 h-12 rounded-lg border flex items-center justify-center shrink-0 bg-black/25",
                  style.ring
                )}
              >
